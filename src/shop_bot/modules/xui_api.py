...
def update_or_create_client_on_panel(api: Api, inbound_id: int, email: str, days_to_add: int | None = None, target_expiry_ms: int | None = None, enable_flow: bool = True) -> tuple[str | None, int | None, str | None]:
    try:
        ...
        # Determine if flow should be used based on inbound protocol and enable_flow setting
        # For VLESS Reality with flow enabled, use xtls-rprx-vision
        # For XHTTP or when flow is disabled, don't use flow
        protocol = getattr(inbound_to_modify, 'protocol', '').lower()
        has_reality = hasattr(inbound_to_modify.stream_settings, 'reality_settings') if hasattr(inbound_to_modify, 'stream_settings') else False
        use_flow = enable_flow and protocol == "vless" and has_reality
        client_flow = "xtls-rprx-vision" if use_flow else None
        ...
        if client_index != -1:
            ...
            # Set or remove flow based on settings
            if client_flow:
                try:
                    inbound_to_modify.settings.clients[client_index].flow = client_flow
                except Exception:
                    pass
            else:
                # Try to clear flow if it should be disabled
                try:
                    if hasattr(inbound_to_modify.settings.clients[client_index], 'flow'):
                        inbound_to_modify.settings.clients[client_index].flow = ""
                except Exception:
                    pass
            ...
            # Set flow only if enabled for Reality protocol
            if client_flow:
                new_client.flow = client_flow
...
async def create_or_update_key_on_host(host_name: str, email: str, days_to_add: int | None = None, expiry_timestamp_ms: int | None = None) -> Dict | None:
    ...
    # Get enable_flow setting from host configuration (default True for backward compatibility)
    enable_flow = bool(host_data.get('enable_flow', 1))
    ...
    client_uuid, new_expiry_ms, client_sub_token = update_or_create_client_on_panel(
        api, inbound.id, email, days_to_add=days_to_add, target_expiry_ms=expiry_timestamp_ms, enable_flow=enable_flow
    )
