...
def create_webhook_app(bot_controller_instance):
    ...
    @login_required
    def add_host_route():
        # If checkbox is checked, request.form.get will return '1', otherwise it won't be present
        enable_flow = request.form.get('enable_flow') == '1'
    ...
    @flask_app.route('/update-host-flow', methods=['POST'])
    @login_required
    def update_host_flow_route():
        host_name = request.form.get('host_name', '').strip()
        # Checkbox sends '1' if checked, otherwise not present (None)
        enable_flow = request.form.get('enable_flow') == '1'
        
        if not host_name:
            flash('Ошибка: хост не указан.', 'danger')
            return redirect(url_for('settings_page', tab='hosts'))
        
        try:
            if database.update_host_flow_setting(host_name, enable_flow):
                status = 'Включен' if enable_flow else 'Отключен'
                flash(f"Flow параметр для хоста '{host_name}' {status}.", 'success')
            else:
                flash(f"Ошибка при обновлении Flow для '{host_name}'.", 'danger')
        except Exception as e:
            logger.error(f"Error updating host flow setting: {e}")
