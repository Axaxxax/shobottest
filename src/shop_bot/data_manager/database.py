...
def run_migration():
    ...
    try:
        ...
        if table_exists:
            ...
            # Flow control for XHTTP compatibility
            if 'enable_flow' not in xh_columns:
                cursor.execute("ALTER TABLE xui_hosts ADD COLUMN enable_flow INTEGER DEFAULT 1")
                logging.info(" -> Столбец 'enable_flow' успешно добавлен в 'xui_hosts'.")
...
def create_host(name: str, url: str, user: str, passwd: str, inbound: int, subscription_url: str | None = None, enable_flow: bool = True):
    try:
        ...
        enable_flow_int = 1 if enable_flow else 0

        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            try:
                cursor.execute(
                    "INSERT INTO xui_hosts (host_name, host_url, host_username, host_pass, host_inbound_id, subscription_url, enable_flow) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (name, url, user, passwd, inbound, subscription_url, enable_flow_int)
                )
...
def update_host_flow_setting(host_name: str, enable_flow: bool) -> bool:
    """Update flow (xtls-rprx-vision) setting for a host."""
    try:
        host_name_n = normalize_host_name(host_name)
        enable_flow_int = 1 if enable_flow else 0
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute("SELECT 1 FROM xui_hosts WHERE TRIM(host_name) = TRIM(?)", (host_name_n,))
            if cursor.fetchone() is None:
                logging.warning(f"update_host_flow_setting: хост не найден '{host_name_n}'")
                return False

            cursor.execute(
                "UPDATE xui_hosts SET enable_flow = ? WHERE TRIM(host_name) = TRIM(?)",
                (enable_flow_int, host_name_n)
            )
            conn.commit()
            logging.info(f"Host '{host_name_n}' flow setting updated to {enable_flow}")
            return True
    except sqlite3.Error as e:
        logging.error(f"Не удалось обновить настройку flow для хоста '{host_name}': {e}")
        return False
